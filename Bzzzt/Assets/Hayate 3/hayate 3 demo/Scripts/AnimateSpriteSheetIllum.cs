...
public class AnimateSpriteSheetIllum : MonoBehaviour {
    ...
    void Start()
    {
		...
		GetComponent<Renderer>().sharedMaterial.SetTextureScale("_Illum", size);
    }
        ...
        while (true)
        {
            for (int i = Rows-1; i >= 0; i--) // y
            {
                ...
                for (int j = 0; j <= Columns-1; j++) // x
                {
					...
					GetComponent<Renderer>().sharedMaterial.SetTextureOffset("_Illum", offset);
